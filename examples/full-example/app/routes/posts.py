...
@router.get("/list")
def list_posts(user=Depends(manager), db=Depends(get_session)) -> List[PostResponse]:
    """Lists all posts of the current user"""
    user = get_user_by_name(user.username, db)
    return [PostResponse.from_orm(p) for p in user.posts]


@router.get("/list/{username}")
def list_posts_for_user(
    username: str, _=Depends(manager), db=Depends(get_session)
) -> List[PostResponse]:
